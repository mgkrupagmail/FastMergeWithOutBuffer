 ...
 */
template<class T>
bool VerifyMergeStability(std::vector<ObjectAndIndex<T>> &vec) {
  bool result = true;
...
bool VerifyMergeStability(int vec_size,
  ...
  auto result = VerifyMergeStability(vec);
  if (!result) {
...
bool VerifyMergeStability(int vec_size,
  ...
  return VerifyMergeStability<T>(vec_size,
...
bool VerifyMergeStability(int vec_size,
  ...
  for (unsigned int i = 0; i < num_tests_per_vec_size; i++) {
    ...
    auto result = VerifyMergeStability<T>(vec_size,
