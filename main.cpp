...
int main() {
  ...
  for (auto vec_size = vec_size_start; vec_size <= vec_size_end; vec_size++)
    TestCorrectnessOfMerge(vec_size, num_tests_per_vec_size,
        should_randomly_pick_start_right, false, 0, 10*vec_size);
  ...
  int start_right = -2; //For the first vector, pick the lengths of the left
                        // and right vectors randomly.
  ...
  // [0, const_to_scale_vec_size_by * vec_size].
  //This is useful for controlling how many repeated values the randomly
  // generated vectors have.
  long double const_to_scale_vec_size_by = 1.2l; // If <= 0 then the random
                          //vector values independently of the vector's size.
  ValueType value_lower_bound = 0;

  TotalTimes grand_total_times;
  auto total_times_exp = TimeMergeFunctions<ValueType>(vec_size_start,
                  start_right, pick_new_random_start_right_for_each_new_vec,
                  next_vec_size_lambda, num_tests_per_vec_size_lambda,
                  num_repititions_per_vec_lambda, print_vec_averages,
                  print_total_averages, verbose, print_vec_original,
                  const_to_scale_vec_size_by, value_lower_bound);

  auto total_times_random = TimeMergeFunctionsOnRandomVecSizes<ValueType>(
                  vec_size_lower_bound, vec_size_upper_bound,
                  number_of_random_vec_sizes,
                  start_right, pick_new_random_start_right_for_each_new_vec,
                  num_tests_per_vec_size_lambda,
